...
public sealed class LibreViesGame : MonoBehaviour
{
    ...
    private readonly Dictionary<Transform, Obstacle> collisionsObjetsEdition =
        new Dictionary<Transform, Obstacle>();
    ...
    private sealed class ElementEdition
    {
        public Transform Root;
    ...
    private bool EstObjetDeplacableEdition(Transform objet)
    {
        if (objet == null || objet == transform || objet == player) return false;
        string nom = objet.name ?? "";
        if (nom == "Porte" || nom == "Fenetre" || nom == "Affiche_Maison") return true;
        if (nom == "Sapin" || nom == "Fontaine" || nom == "Panneau"
            || nom == "Lampadaire" || nom == "Banniere" || nom == "Baril"
            || nom == "Caisse" || nom == "Garde"
            || nom.StartsWith("Asset_CC0_") || nom.StartsWith("PNJ_")
            || nom == "Araignee" || nom == "Souris" || nom == "Rat") return true;
        return false;
    }

    private void ConstruireObjetsEdition()
    {
        objetsEdition.Clear();
        var dejaAjoutes = new HashSet<Transform>();
        ...
        for (int i = 0; i < transforms.Length; i++)
        {
            ...
            dejaAjoutes.Add(objet);
        }
        Journal("edition : " + objetsEdition.Count + " objet(s) deplacable(s)");
    }

    private string CheminObjetEdition(Transform objet)
    {
        if (objet == null) return "";
        var morceaux = new List<string>();
        Transform courant = objet;
        while (courant != null)
        {
            morceaux.Add(courant.name + "[" + courant.GetSiblingIndex() + "]");
            courant = courant.parent;
        }
        morceaux.Reverse();
        return string.Join("/", morceaux.ToArray());
    }
    ...
    {
        Transform[] transforms = FindObjectsByType<Transform>();
        var objets = new List<Transform>();
        var dejaAjoutes = new HashSet<Transform>();
        for (int i = 0; i < transforms.Length; i++)
        {
            if (RacineAEnregistrer(transforms[i]))
            {
                objets.Add(transforms[i]);
                dejaAjoutes.Add(transforms[i]);
            }
        }
        for (int i = 0; i < objetsEdition.Count; i++)
        {
            Transform objet = objetsEdition[i].Root;
            if (objet != null && !dejaAjoutes.Contains(objet))
            {
                objets.Add(objet);
                dejaAjoutes.Add(objet);
            }
    ...
    private void SynchroniserTousLesBatiments()
    {
        ...
        for (int i = 0; i < objetsEdition.Count; i++)
        {
            ...
            if (element.Maison == null && element.Root != null)
                element.SolY = element.Root.position.y;
        }
    }

    private Transform TrouverRacineCoordonnee(string nom, string chemin,
        Vector3 position, HashSet<Transform> utilisees)
    {
        Transform meilleur = null;
        float meilleureDistance = float.MaxValue;
        List<Transform> objets = ObjetsCoordonneesEdition();
        for (int i = 0; i < objets.Count; i++)
        {
            Transform objet = objets[i];
            if (utilisees.Contains(objet)) continue;
            if (!string.IsNullOrEmpty(chemin) && CheminObjetEdition(objet) == chemin)
                return objet;
        }
        for (int i = 0; i < objets.Count; i++)
        {
            Transform objet = objets[i];
            if (objet.name != nom || utilisees.Contains(objet)) continue;
            float distance = (objet.position - position).sqrMagnitude;
        ...
        try
        {
            ...
            for (int i = 0; i < fichier.Objets.Count; i++)
            {
                ...
                Transform objet = TrouverRacineCoordonnee(coordonnee.Nom,
                    coordonnee.Chemin, coordonnee.Position, utilisees);
                ...
                if (element != null && element.Maison != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(coordonnee.MateriauFenetre))
                        AppliquerMateriauMaison(element.Maison, "fenetre", coordonnee.MateriauFenetre);
                    if (!string.IsNullOrEmpty(coordonnee.MateriauPorte))
    ...
    // ------------------------------------------------------------------
    private Obstacle ColCercle(float x, float z, float r, float h = HauteurCollision)
    {
    ...
    private void CreateBuilding(Vector3 position, Vector3 size, string name)
    {
        ...
        // La dalle s'arrete au milieu de l'epaisseur du mur : elle penetre
        // dans le mur pour supprimer le jour, mais ne depasse jamais a
        // l'exterieur. Elle reste donc invisible depuis l'exterieur.
        Box(new Vector3(0f, 0.31f, 0f),
            new Vector3(Mathf.Max(size.x - EpaisseurMurInterieur, 1.2f), 0.08f,
                Mathf.Max(size.z - EpaisseurMurInterieur, 1.2f)), "Wood", root, "Sol_Interieur");
    ...
    private void ModifierTailleMur(ElementEdition element, int cote, bool agrandir)
    {
        ...
        bool axeX = cote == -1 || cote == 1;
        float ancienneDimension = axeX ? batiment.Largeur : batiment.Profondeur;
        float nouvelleDimension = Mathf.Max(2.5f,
            ancienneDimension + (agrandir ? 0.5f : -0.5f));
        if (Mathf.Abs(nouvelleDimension - ancienneDimension) < 0.001f) return;
        Vector3 echelle = batiment.Root.localScale;
        if (axeX)
            echelle.x *= nouvelleDimension / ancienneDimension;
        else
            echelle.z *= nouvelleDimension / ancienneDimension;
        batiment.Root.localScale = echelle;
        float signe = cote == -1 || cote == -2 ? -1f : 1f;
        Vector3 axeMonde = (batiment.Root.rotation
            * (axeX ? Vector3.right : Vector3.forward)).normalized;
        batiment.Root.position += axeMonde * signe
            * ((nouvelleDimension - ancienneDimension) * 0.5f);
        if (axeX) batiment.Largeur = nouvelleDimension;
        else batiment.Profondeur = nouvelleDimension;
        ActualiserDimensionsBatiment(batiment);
        SynchroniserBatiment(batiment);
        ...
        ShowInfo((agrandir ? "Mur agrandi de 0,5 m : " : "Mur reduit de 0,5 m : ")
            + NomCoteRedimensionnement(cote));
    }
    ...
    private void AppliquerMateriauMaison(Batiment batiment, string cible, string materiau)
    {
        ...
        else if (cible == "toit") batiment.MateriauToit = materiau;
        Material materiel = Mat(materiau);
    ...
    private void DessinerChoixMateriaux(Batiment batiment, string cible,
        string[] libelles, string[] materiaux, float y)
    {
        GUI.Label(new Rect(34f, y, 150f, 24f), "Couleur / revetement :", smallStyle);
        for (int i = 0; i < libelles.Length; i++)
        {
            float x = 174f + i * 59f;
            if (GUI.Button(new Rect(x, y - 2f, 54f, 28f), libelles[i], buttonStyle))
                AppliquerMateriauMaison(batiment, cible, materiaux[i]);
        }
    }
    ...
    private void DessinerEditionMaison()
    {
        ...
        if (GUI.Button(new Rect(cadre.x + 16f, cadre.y + 56f, 190f, 30f),
            "DEPLACER LA MAISON", buttonStyle)) editionOutilMaison = 0;
        if (GUI.Button(new Rect(cadre.x + 216f, cadre.y + 56f, 190f, 30f),
            "MODIFIER LES MURS", buttonStyle)) editionOutilMaison = 1;
        if (GUI.Button(new Rect(cadre.x + 16f, cadre.y + 91f, 190f, 30f),
            "MODIFIER LES FENETRES", buttonStyle)) editionOutilMaison = 2;
        if (GUI.Button(new Rect(cadre.x + 216f, cadre.y + 91f, 190f, 30f),
        ...
        if (editionOutilMaison == 0)
        {
            GUI.Label(new Rect(cadre.x + 16f, cadre.y + 176f, 390f, 54f),
                "Maintenez le clic sur la maison pour la deplacer.\n"
                    + "Les autres boutons ne deplacent pas la maison.", smallStyle);
            return;
        }
        if (editionOutilMaison == 1)
        {
    ...
    private bool TryZoneEcranElementEdition(ElementEdition element,
        out Rect zone, out float profondeur)
    {
        zone = new Rect();
        profondeur = float.MaxValue;
        if (element == null || element.Root == null || !element.Root.gameObject.activeInHierarchy)
            return false;
        Vector3[] points;
        if (element.Maison != null)
        {
        ...
        {
            Renderer[] rendus = element.Root.GetComponentsInChildren<Renderer>(true);
            Bounds limites = new Bounds(element.Root.position, Vector3.one * 0.8f);
            if (rendus.Length > 0)
            {
                limites = rendus[0].bounds;
                for (int i = 1; i < rendus.Length; i++) limites.Encapsulate(rendus[i].bounds);
            }
            Vector3 min = limites.min;
            Vector3 max = limites.max;
            points = new Vector3[]
            {
        ...
        {
            Vector3 ecran = gameCamera.WorldToScreenPoint(points[i]);
            if (ecran.z <= 0f)
            {
                visible = false;
                break;
        ...
        float marge = element.Maison != null ? 18f : 10f;
        zone = Rect.MinMaxRect(minX - marge, minY - marge, maxX + marge, maxY + marge);
        return true;
    }
    ...
    private void SynchroniserElementEdition(ElementEdition element)
    {
        ...
        if (element.Collision != null)
        {
            ...
            element.Collision.Z = element.Root.position.z;
        }
    }
    ...
    private void CommencerDeplacementMaisonEdition()
    {
        ...
        ShowInfo(cible.Type + " souleve : maintenez le clic et deplacez-le");
    }
    ...
    private void BasculerModeEdition()
    {
        if (modeEdition)
        {
            if (editionMaisonEnDeplacement)
                TerminerDeplacementMaisonEdition();
            else if (elementEditionSelectionne != null)
                PoserElementEdition(elementEditionSelectionne);
    ...
    private bool CreateNatureInstance(string asset, float x, float z, float scale, bool collision)
    {
        ...
        if (collision)
        {
            if (asset.StartsWith("CommonTree") || asset.StartsWith("Pine"))
            {
                ...
                tronc.radius = 0.65f;
                tronc.height = 5.2f;
                collisionNature = ColCercle(x, z, 0.72f * scale, 6.8f * scale);
            }
            else if (asset.StartsWith("Rock"))
            {
                ...
                collisionNature = ColCercle(x, z, 1.0f * scale, 2.0f * scale);
            }
        }
        if (collisionNature != null) collisionsObjetsEdition[instance.transform] = collisionNature;
        return true;
    }
    ...
    private void CreatePine(float x, float z, float taille)
    {
        ...
        collisionsObjetsEdition[root] = ColCercle(x, z, 0.4f * taille, 2.2f);   // tronc
    }
